...
\begin{frame}{Extraction de la grammaire (arbre quotient)}
    ...
    \end{columns}
    \vspace{1em}
    \onslide<4->{\begin{block}{Grammaire}
        \vspace{-1em}
        \begin{alignat*}{6}
    ...
    \begin{columns}
        \begin{column}{.7\textwidth}
            \only<-5>{\begin{description}
                ...
                \uncover<5->{\item[Collection] Un ensemble de groupes équivalents.}
            \end{description}}
        ...
        \begin{column}{.3\textwidth}
            \begin{center}
                \begin{tikzpicture}[-latex,on grid]
                    ...
                    \onslide<2->{\node[above = of v2] (b) {b};}
                    \onslide<2->{\node[above = of v3] (c) {c};}
                    \onslide<3->{\node[above = of a] (g1) {$Grp_1$};}
                    \onslide<3->{\node[above = of c] (g2) {$Grp_2$};}
                    \onslide<4->{\node[above right = of g1] (r) {$Rel_1$};}
                    \onslide<5->{\node[above = of r] (cr) {$Coll_{3}$};}
                    \onslide<5->{\node[left = of cr] (c1) {$Coll_1$};}
                    \onslide<5->{\node[right = of cr] (c2) {$Coll_2$};}
...
\begin{frame}{Exemple}
    ...
    \only<2>{\begin{adjustbox}{max width=\linewidth}
            \begin{forest}
                where n children=0{tier=word}{}
                [. [Person [SEX] [AGE]] [Condition [SOSY] [DURATION]]]
            \end{forest}
        \end{adjustbox}
    ...
    \only<4>{\begin{adjustbox}{max width=\linewidth}
        ...
        \begin{adjustbox}{max width=\linewidth}
            \begin{forest}
                where n children=0{tier=word}{}
                [. [Person [SEX]] [Condition [SOSY] [DURATION]]]
            \end{forest}
        \end{adjustbox}}

    \only<5>{\begin{adjustbox}{max width=\linewidth}
            \begin{forest}
                where n children=0{tier=word}{}
                [hasCondition [Person [SEX] [AGE]] [Condition [SOSY] [DURATION]]]
            \end{forest}
        \end{adjustbox}
        ...
        \begin{adjustbox}{max width=\linewidth}
            \begin{forest}
                where n children=0{tier=word}{}
                [hasCondition [Person [SEX]] [Condition [SOSY] [DURATION]]]
            \end{forest}
        \end{adjustbox}}

    \only<6>{\begin{adjustbox}{max width=\linewidth}
        \begin{forest}
            where n children=0{tier=word}{}
            [hasCondition [Person [SEX] [AGE]] [Condition [SOSY] [DURATION]]]
        \end{forest}\qquad
    ...
    \begin{block}{}
        \begin{alignat*}{4}
            ROOT         & \to hasCondition       && hasCondition & \to Person ~ Condition \\
            Person       & \to SEX ~ AGE          && Condition    & \to SOSY ~ DURATION
        \end{alignat*}
    \end{block}}
    ...
    \begin{block}{}
        \begin{alignat*}{4}
            ROOT         & \to hasCondition && \qquad hasCondition & \to Person ~ Condition \\
            Person       & \to SEX ~ AGE    && \qquad Condition    & \to SOSY ~ DURATION
        \end{alignat*}
    \end{block}}
\end{frame}
