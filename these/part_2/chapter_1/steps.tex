...
\begin{procedure}[htb]
    \caption{enrich\_tree($T$ = ($D$, $l$), $Ents$, $Rels$)}
    \label{algo:struct:inslist}
    ...
    \ForEach{$(entPos_1, entPos_2, rel) \in Rels$}{
        \insrel{$T$, $entPos_1$, $entPos_2$, $rel$}\label{algo:struct:inslist:rel}\;
    }

    \tcp{Simplification des arbres}
    \ForEach{$p \in D$ tant que $\textsf{height}(p) = 1$ et $l(p) \neq \textsf{ENT}$}{
        \delelem{$T$, $l$}\label{algo:struct:inslist:del}\;
    }
    \ForEach{$p \in D$}{
        \reduce{$T$, $p$, $\emptyset$}\label{algo:struct:inslist:reduce}\;
    }
\end{procedure}
...
%L'arbre quotient est ensuite construit à partir de la fonction \ref{algo:struct:quotient:QDom}.
...
\begin{function}[htb]
    ...
    \While{$classes \neq \emptyset$}{
        ...
        $classes \gets classes \setminus \{(C, u)\}$\;
