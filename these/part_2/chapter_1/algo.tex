...
\begin{procedure}[htb]
    ...
    \While{$i \leq \text{epochs}$ and $\simp = \textsc{True}$}{
        ...
        \If{$\simp = \textsc{False}$}{
            \ForEach{$p \in D_i$ tel que $depth(p) = 1$\label{algo:struct:rewrite:reduce-top}}{
        ...
        \If{$\simp = \textsc{False}$}{
            \ForEach{$u.j \in D_i$ où $t(u.j) = `ENT$'\label{algo:struct:rewrite:reduce-bot}}{
...
\begin{procedure}
    ...
    \While{$equivalent\_st \neq \emptyset$}{
        ...
        \ForEach{$st \in ST\_min$ tel que $\{e \mid e \in st, t(e) \neq `GROUP`\} = \emptyset$}{
            $j \gets 0$\;
...
\begin{procedure}
    ...
    \ForAll{sous-arbre \st tel que $\st = T|_u$ and $t(u) \notin \{`ENT`, `GROUP`, `REL`, `COLL`\}$}{
