...
\begin{align*}
    \mathcal{D} = \{ & Patient(N_1, \text{homme}, 78), Exam(N_1, N_2, N_3, \text{01-01-01}), Parameter(N_3, \text{température}, N_4), \\
                     & Reveal(N_3, \text{fièvre}), hasPatho(N_1, \text{cancer}), getTreatment(N_1, \text{résection}), forPatho(\text{résection}, \text{cancer}), \\
                     & leadsTo(\text{cancer}, \text{métastase}), concernAna(\text{cancer}, \text{prostate}), Anatomie(\text{prostate}) \}
\end{align*}

La figure~\ref{fig:update:db:runex} montre un extrait de l'instance donnée en figure~\ref{fig:runex:graph} (page~\pageref{fig:runex:graph}) correspondant à l'ensemble de faits $\mathcal{D}$ et suivant le modèle définit.
...
\begin{figure}[htb]
    ...
    \begin{adjustbox}{varwidth=\linewidth,max height=\textheight}
        ...
        \end{tikzpicture}
    \end{adjustbox}
    \caption[Extrait de l'instance de la base de données Graphe]{Extrait de l'instance de la base de données Graphe. Les nœuds ellipse représentent les \textit{Éléments} et les rectangles les \textit{Atomes}. Les labels et les propriétés d'optimisation ne sont pas affichés}
